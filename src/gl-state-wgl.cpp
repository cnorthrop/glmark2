...
GLStateWGL::api_proc GLStateWGL::load_proc(const char *name, void *userptr)
{
    ...
    void* proc = state->wgl_library_.load(name);
    if (proc) {
        return reinterpret_cast<GLStateWGL::api_proc>(proc);
    }

    auto get_proc_addr = reinterpret_cast<decltype(&wglGetProcAddress)>(state->get_proc_addr_);
    return reinterpret_cast<GLStateWGL::api_proc>(get_proc_addr(name));
}
